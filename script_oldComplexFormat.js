...
window.KanbanPlugin = (function() {
    ...
    function generateCardHtml(cardData) {
        ...
        if (cardData.lastModifiedBy && cardData.lastModified && 
            ...
            if (!isNaN(modifiedDate.getTime())) {
                html += `<span class="kanban-last-modified">✏️ ${escapeHtml(cardData.lastModifiedBy)} • ${modifiedDate.toLocaleDateString('fr-FR')}</span>`;
            }
        }
    ...
    function updateCardDisplay(cardElement, cardData) {
        ...
        updateCardTags(cardElement, cardData);
        
        // Update footer with assignee and last modified
        updateCardFooter(cardElement, cardData);
        
        // Update creator info (sous le titre)
        updateCardCreator(cardElement, cardData);
    ...
    function updateCardFooter(cardElement, cardData) {
        ...
        if (cardData.lastModifiedBy && cardData.lastModified && 
            ...
            const modifiedDate = new Date(cardData.lastModified);
            if (!isNaN(modifiedDate.getTime())) {
                footer.innerHTML += `<span class="kanban-last-modified">✏️ ${escapeHtml(cardData.lastModifiedBy)} • ${modifiedDate.toLocaleDateString('fr-FR')}</span>`;
            }
        }
    }

    /**
     * Update card tags (au-dessus du titre)
     */
    function updateCardTags(cardElement, cardData) {
        // Supprimer les tags existants
        const existingTags = cardElement.querySelector('.kanban-card-tags');
        if (existingTags) {
            existingTags.remove();
        }
        
        // Ajouter nouveaux tags si présents
        if (cardData.tags && cardData.tags.length > 0) {
            const tagsElement = document.createElement('div');
            tagsElement.className = 'kanban-card-tags';
            
            cardData.tags.forEach(tag => {
                if (tag.trim()) {
                    tagsElement.innerHTML += `<span class="kanban-tag">${escapeHtml(tag.trim())}</span>`;
                }
            });
            
            // Insérer après les actions, avant le header
            const header = cardElement.querySelector('.kanban-card-header');
            if (header) {
                header.before(tagsElement);
            } else {
                const actions = cardElement.querySelector('.kanban-card-actions');
                if (actions) {
                    actions.after(tagsElement);
                } else {
                    cardElement.prepend(tagsElement);
                }
            }
        }
    }
    ...
    /**
     * Update card creator info (sous le titre)
     */
    function updateCardCreator(cardElement, cardData) {
        // Supprimer créateur existant
        const existingCreator = cardElement.querySelector('.kanban-card-creator');
        if (existingCreator) {
            existingCreator.remove();
        }
        
        // Ajouter créateur si présent
        if (cardData.creator) {
            const creatorElement = document.createElement('div');
            creatorElement.className = 'kanban-card-creator';
            
            const creatorInitial = cardData.creator.charAt(0).toUpperCase();
            let creatorHtml = `<div class="kanban-card-avatar">${creatorInitial}</div>`;
            creatorHtml += `<span>Par ${escapeHtml(cardData.creator)}</span>`;
            
            if (cardData.created) {
                const createdDate = new Date(cardData.created).toLocaleDateString('fr-FR');
                creatorHtml += `<span class="kanban-card-date"> • ${createdDate}</span>`;
            }
    ...
    function updateCardDueDate(cardElement, cardData) {
        ...
        if (cardData.dueDate && cardData.dueDate.trim() !== '') {
            ...
            const footerElement = cardElement.querySelector('.kanban-card-footer');
            if (footerElement) {
                footerElement.before(dueDateElement);
            } else {
    ...
    function extractBoardData(board) {
        ...
        board.querySelectorAll('.kanban-column').forEach((column, index) => {
            ...
            column.querySelectorAll('.kanban-card').forEach(card => {
                ...
                // Extract assignee from footer (nouveau format avec emoji)
                const assigneeElement = card.querySelector('.kanban-assignee');
                if (assigneeElement) {
                    // Supprimer l'emoji 👤 du début si présent
                    cardData.assignee = assigneeElement.textContent.trim().replace(/^👤\s*/, '');
                }
                
                // Extract tags from new position (kanban-card-tags)
                const tagElements = card.querySelectorAll('.kanban-card-tags .kanban-tag');
                cardData.tags = Array.from(tagElements).map(tag => tag.textContent.trim());
                
                // Extract creator from new position (sous le titre)
                const creatorElement = card.querySelector('.kanban-card-creator span:first-of-type');
                if (creatorElement) {
                    // Nettoyer le texte en supprimant tous les "Par " multiples
                    let creatorText = creatorElement.textContent.trim();
                    // Supprimer tous les "Par " du début (multiples ou simples)
                    creatorText = creatorText.replace(/^(Par\s+)+/gi, '');
                    cardData.creator = creatorText;
                }
                
                // Extract created date from new position (dans creator)
                const dateElement = card.querySelector('.kanban-card-creator .kanban-card-date');
                if (dateElement) {
                    // Supprimer le " • " du début si présent et nettoyer
                    let dateText = dateElement.textContent.trim().replace(/^•\s*/, '');
                    // Vérifier si c'est une date valide
                    if (dateText && dateText !== 'Invalid Date') {
                        cardData.created = dateText;
                    }
                }
                ...
                // Extract last modified info from footer (nouveau format)
                const lastModifiedElement = card.querySelector('.kanban-card-footer .kanban-last-modified');
                if (lastModifiedElement) {
                    const text = lastModifiedElement.textContent.trim();
                    // Format: "✏️ Username • dd/mm/yyyy"
                    const match = text.match(/^✏️\s*(.+?)\s*•\s*(.+)$/);
                    if (match) {
                        cardData.lastModifiedBy = match[1].trim();
                        cardData.lastModified = match[2].trim();
                    }
                }
