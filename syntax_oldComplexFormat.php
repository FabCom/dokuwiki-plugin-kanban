...
class syntax_plugin_kanban extends SyntaxPlugin
{
    ...
    private function renderCard($renderer, $card)
    {
        ...
        if (!empty($card['tags'])) {
            $renderer->doc .= '<div class="kanban-card-tags">';
            foreach ($card['tags'] as $tag) {
                if (!empty(trim($tag))) {
                    $renderer->doc .= '<span class="kanban-tag">' . htmlspecialchars(trim($tag)) . '</span>';
                }
            }
            $renderer->doc .= '</div>';
        }
        ...
        if (!empty($card['creator'])) {
            ...
            $renderer->doc .= '<span>Par ' . htmlspecialchars($card['creator']) . '</span>';
            ...
            if (!empty($card['created'])) {
                ...
                $renderer->doc .= '<span class="kanban-card-date"> • ' . $createdDate . '</span>';
            }
            $renderer->doc .= '</div>';
        }
        ...
        if (!empty($card['dueDate'])) {
            $renderer->doc .= '<div class="kanban-card-due-date">';
            ...
            if (preg_match('/^\d{4}-\d{2}-\d{2}/', $dueDate)) {
                ...
                $isoDate = $dueDate;
            } elseif (preg_match('/^\d{2}\/\d{2}\/\d{4}/', $dueDate)) {
                // dd/mm/yyyy format
                $dueDateFormatted = $dueDate;
                // Convert to ISO for data attribute
            ...
            $renderer->doc .= '<span class="due-date" data-date="' . htmlspecialchars($isoDate) . '">📅 Échéance: ' . htmlspecialchars($dueDateFormatted) . '</span>';
            $renderer->doc .= '</div>';
        }
        ...
        if (!empty($card['lastModifiedBy']) && !empty($card['lastModified']) && 
            $card['lastModified'] !== $card['created']) {
            // Parse date if it's in format
            $modifiedDate = $card['lastModified'];
            if (preg_match('/^\d{4}-\d{2}-\d{2}/', $modifiedDate)) {
                $modifiedDate = date('d/m/Y', strtotime($modifiedDate));
            }
            $renderer->doc .= '<span class="kanban-last-modified">✏️ ' . htmlspecialchars($card['lastModifiedBy']) . ' • ' . htmlspecialchars($modifiedDate) . '</span>';
        }
